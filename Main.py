...
def assumed_fill(game: Game) -> tuple[bool]:
    ...
    while fill_algorithm.count_items_remaining():
        ...
        if fill_algorithm.count_items_remaining() == 0:
            ...
            completable = ((game.all_locations["Morph Ball"]["item"] == Items.Morph) or (game.all_locations["Construction Zone Missile"]["item"] == Items.Morph))
            #completable = True
            if completable:
...
def write_rom(game: Game, romWriter: Optional[RomWriter] = None) -> str:
    ...
    rom_name = f"Cliff{game.seed}.sfc"
    rom1_path = f"roms/{rom_name}"
    rom_clean_path = "roms/CliffhangerRedux.sfc"
